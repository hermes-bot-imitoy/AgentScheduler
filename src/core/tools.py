...
def create_talk_toolkit(pool: Any) -> ToolKit:
    """Communication toolkit: inter-role talk tool.

    Args:
        pool: The RolePool instance (for looking up target roles).

    Returns:
        ToolKit with the 'talk' tool ready to use.
    """
    from src.core.roles import Urgency, Task

    tk = ToolKit(name="communication", description="Inter-role communication")
    ...
    def _talk_handler(args: dict) -> str:
    ...
    tk.add_python_tool(
        ...
        input_schema={
            ...
            "properties": {
                "target": {
                    "type": "string",
                    "description": f"Target person's role_id. Available: {', '.join(pool.list_roles())}",
                },
                "message": {
                    "type": "string",
