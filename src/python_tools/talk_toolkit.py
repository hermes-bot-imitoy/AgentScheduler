...
def create_talk_toolkit(pool: Any, drive: Any = None) -> ToolKit:
    """创建通信工具类 (talk + list_roles).

    参数:
        pool:  RolePool 实例, 用于查找目标角色并投递任务.
        drive: DriveStore 实例 (可选). 提供后 talk 支持 attachment 参数:
              附件为企业云盘路径, 发送时校验存在且可读.
    ...
    tk._drive = drive  # type: ignore[attr-defined]  # 附件校验用 (企业云盘)
    ...
    def _talk_handler(args: dict[str, Any]) -> str:
        ...
        # attachment: 企业云盘路径 (指向公共资源, 发送时校验存在且可读)
        ...
        # ── 附件校验: 云盘路径必须存在 (文件) 且当前角色可读 ──
        if attachment is not None:
            drive = getattr(tk, "_drive", None)
            if drive is None:
                return "错误: 云盘不可用, 无法发送附件 (attachment)."
            if not drive.file_exists(attachment):
                return f"错误: 附件无效: 云盘文件不存在 '{attachment}'"
            try:
                drive.read(sender.name if sender is not None else "",
                           attachment)
            except (ValueError, PermissionError) as exc:
                return f"错误: 附件无效: {exc}"
    ...
    tk.add_python_tool(
        ...
        description=(
            ...
            "attachment 可选: 企业云盘文件路径 (如 '郭晓东/设计稿.md'), "
            "发送给对方共享资源, 对方可用 drive_read 读取; 发送前系统会校验文件存在.\n"
        ...
        input_schema={
            ...
            "properties": {
                ...
                "attachment": {
                    "type": "string",
                    "description": "可选: 企业云盘文件路径 (如 '郭晓东/设计稿.md'), 作为附件随消息发送",
                },
            },
