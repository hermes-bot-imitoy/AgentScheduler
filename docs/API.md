...
**交流**: `talk_to(target, message, urgency="NORMAL") -> str`（编程式跨角色消息）; LLM 经 talk 工具（target/message/urgency/wait）通信, `wait=true` 时发送方进入 WAIT 状态同步等待对方回复（回复用 talk 即可, 120s 超时, 互等死锁自动检测拒绝）。
